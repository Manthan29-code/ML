...
# ── Try to import pandas, numpy, scipy, and sklearn ───────────────────────────
try:
    import pandas as pd
    import numpy as np
    HAS_PANDAS = True
...
def analyse_with_pandas(csv_path):
    ...
    # ── 9. Advanced Non-Visual Diagnostics ────────────────────────────────────
    lines.append(section("9. ADVANCED NON-VISUAL DIAGNOSTICS"))
    diagnostic_warnings = []
    
    if num_cols:
        lines.append("  ── Distribution Shape & Artifact Checks ──")
        for col in num_cols:
            s = df[col].dropna()
            if len(s) < 10:
                continue
            ...
            # Check Multimodality via Kurtosis
            kurt = float(s.kurtosis())
            if kurt < -0.7:
                col_warnings.append(f"⚠️ MULTIMODAL/FLAT PATTERN (Excess Kurtosis: {kurt:.2f} < -0.7)")
            
            # Check Boundary Capping
            min_pct = (s == s.min()).mean()
            max_pct = (s == s.max()).mean()
            if min_pct > 0.03 and s.min() != 0:
                col_warnings.append(f"⚠️ ARTIFICIAL MIN CAP ({min_pct*100:.1f}% values sit exactly at min: {s.min()})")
            if max_pct > 0.03:
                col_warnings.append(f"⚠️ ARTIFICIAL MAX CAP ({max_pct*100:.1f}% values sit exactly at max: {s.max()})")
                
            # Check Rounding Artifacts
            if s.dtype == 'float64':
                whole_num_pct = ((s % 1) == 0).mean()
                if whole_num_pct > 0.80:
                    col_warnings.append(f"⚠️ HEAVY ROUNDING ({whole_num_pct*100:.1f}% of floats are exact integers)")
                    
            if col_warnings:
                lines.append(f"  ── {col} ──")
                for w in col_warnings:
                    lines.append(f"     {w}")
                    diagnostic_warnings.append(f"'{col}': {w}")
                lines.append("")

        # Check Non-Linear Dependencies via Mutual Information
        if HAS_SKLEARN and len(num_cols) >= 2:
            lines.append("  ── Non-Linear Relationship Screening (Mutual Info vs Pearson r) ──")
            ...
            for i in range(len(num_cols)):
                for j in range(i + 1, len(num_cols)):
                    ...
                    p_corr = clean_df[c1].corr(clean_df[c2])
                    mi = mutual_info_regression(clean_df[[c1]], clean_df[c2], random_state=42)[0]
                    
                    if abs(p_corr) < 0.30 and mi > 0.15:
                        lines.append(f"  ⚠️ NON-LINEAR DEPENDENCY: {c1} ↔ {c2} (Pearson r = {p_corr:.2f}, Mutual Info = {mi:.2f})")
                        diagnostic_warnings.append(f"Non-linear pair '{c1}' ↔ '{c2}' (r={p_corr:.2f}, MI={mi:.2f})")
                        found_nonlinear = True
            if not found_nonlinear:
                lines.append("  ✅ No strong non-linear relationships detected.")
    else:
        lines.append("  No numeric columns available for advanced diagnostics.")

    # ── 10. Data quality summary ──────────────────────────────────────────────
    lines.append(section("10. DATA QUALITY SUMMARY & RECOMMENDATIONS"))
    issues = []
    ...
    for col in cat_cols:
        ...
        if unique_ratio > 0.9:
            issues.append(f"  抓  '{col}' has very high cardinality ({df[col].nunique()} unique) → might be an ID column")

    for diag in diagnostic_warnings:
        issues.append(f"  ⚠️  [Diagnostic Warning] {diag}")
    ...
    lines.append(f"  Tool               : csv_analyser.py  (pandas mode with advanced diagnostics)")
...
def main():
    ...
    print(f"\n📊  Analysing: {csv_path}")
    print(f"   Mode: {'pandas (full analysis + advanced diagnostics)' if HAS_PANDAS else 'stdlib (basic analysis — pip install pandas for full)'}")
    ...
    print(f"\n   👉 Now send '{out_txt}' to your assistant instead of the raw CSV.\n")
